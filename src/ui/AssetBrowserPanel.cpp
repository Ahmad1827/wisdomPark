...
AssetBrowserPanel::AssetBrowserPanel(AssetManager& am, const sf::Font& f)
    ...
    selectedAssetId(""), position(1440.f, 78.f), size(410.f, 560.f), isVisible(false), isDraggingAsset(false),
...
void AssetBrowserPanel::draw(sf::RenderWindow& window) {
    ...
    WisdomUI::Theme::DrawCrispText(window, font, ":: ASSET VAULT ::", 13, headerGrip.left + headerGrip.width / 2.0f, headerGrip.top + headerGrip.height / 2.0f, WisdomUI::Theme::SunsetAmber, sf::Color(14, 6, 20), true, true);

    sf::Vector2f mPos = window.mapPixelToCoords(sf::Mouse::getPosition(window));

    importBtnBounds = sf::FloatRect(position.x + size.x - 96.f, position.y + 38.f, 84.f, 26.f);
    WisdomUI::Theme::DrawSunsetButton(window, importBtnBounds, "Import", font, 12, false, importBtnBounds.contains(mPos), true, 1.0f);
    ...
    float catY = position.y + 70.f;
    for (const auto& cat : cats) {
        bool isSelected = (currentCategory == cat.second);
        sf::FloatRect cBounds(position.x + 10.f, catY, 92.f, 26.f);
        categoryBounds.push_back({ cBounds, cat.second });
        WisdomUI::Theme::DrawSunsetButton(window, cBounds, cat.first, font, 11, isSelected, cBounds.contains(mPos), isSelected, 1.0f);
        catY += 31.f;
    }

    gridAreaBounds = sf::FloatRect(position.x + 108.f, position.y + 70.f, size.x - 118.f, size.y - 156.f);
    ...
    float totalGridHeight = std::ceil(static_cast<float>(assets.size()) / 2.0f) * 112.0f;
    maxScrollY = std::max(0.0f, totalGridHeight - gridAreaBounds.height);
    scrollY = std::clamp(scrollY, 0.0f, maxScrollY);

    for (size_t i = 0; i < assets.size(); ++i) {
        float ax = startX + (i % 2) * 142.f;
        float ay = startY + (i / 2) * 112.f;

        if (ay + 102.f < gridAreaBounds.top || ay > gridAreaBounds.top + gridAreaBounds.height) {
            continue;
        }

        bool isSelected = (assets[i]->id == selectedAssetId);

        sf::FloatRect cardRect(ax, ay, 136.f, 102.f);
        ...
        sf::RectangleShape thumb(sf::Vector2f(126.f, 70.f));
        ...
        sf::FloatRect delRect(ax + cardRect.width - 22.f, ay + 4.f, 18.f, 18.f);
        ...
        WisdomUI::Theme::DrawCrispText(window, font, nameStr, 11, ax + 68.f, ay + 86.f, isSelected ? WisdomUI::Theme::SunsetGold : WisdomUI::Theme::TextSecondary, sf::Color::Transparent, true, true);
    }

    if (maxScrollY > 0.0f) {
        float scrollTrackH = gridAreaBounds.height;
        float thumbH = std::max(22.0f, (gridAreaBounds.height / totalGridHeight) * scrollTrackH);
        float thumbY = gridAreaBounds.top + (scrollY / maxScrollY) * (scrollTrackH - thumbH);

        sf::RectangleShape scrollTrack(sf::Vector2f(5.f, scrollTrackH));
        ...
        sf::RectangleShape scrollThumb(sf::Vector2f(5.f, thumbH));
    ...
    sf::FloatRect propArea(position.x + 10.f, position.y + size.y - 78.f, size.x - 20.f, 68.f);
    ...
    if (selectedAsset) {
        ...
        WisdomUI::Theme::DrawCrispText(window, font, info, 12, propArea.left + 10.f, propArea.top + 10.f, WisdomUI::Theme::SunsetAmber);
        WisdomUI::Theme::DrawCrispText(window, font, "Drag asset directly onto canvas to place", 11, propArea.left + 10.f, propArea.top + 32.f, WisdomUI::Theme::TextSecondary);
    }
    else {
        WisdomUI::Theme::DrawCrispText(window, font, "No asset selected", 12, propArea.left + 10.f, propArea.top + 26.f, WisdomUI::Theme::SunsetPlum);
    }
}
...
bool AssetBrowserPanel::handleEvent(const sf::Event& event, const sf::RenderWindow& window, Canvas& canvas, int currentFrame) {
    ...
    if (event.type == sf::Event::MouseButtonPressed && event.mouseButton.button == sf::Mouse::Left) {
        ...
        for (size_t i = 0; i < assets.size(); ++i) {
            ...
            if (sf::FloatRect(ax, ay, 136.f, 102.f).contains(mousePos)) {
